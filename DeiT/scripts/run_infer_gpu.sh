...
display_usage() {
    echo -e "Usage: $0 DATA [--checkpoint CHECKPOINT] [--model ARCHITECTURE] [--output OUTPUT_JSON_FILE (default: $OUTPUT_JSON_FILE)]"
}
...
python3 "$INFER_SCRIPT_DIR/infer.py" "$DATA" --checkpoint "$CHECKPOINT" \
  --model "$ARCHITECTURE" --output "$OUTPUT_JSON_FILE" 2>&1 | tee infer.log

...
from mindspore.ops import ResizeBilinearV2
...
def evaluate(model, testloader, num_class, fixed_test_size=True, verbose=True):
    ...
    for ti in test_iter:
        ...
        interp = ResizeBilinearV2(align_corners=True)
        pred_main = model(Tensor(image, mstype.float32))[1]
        if not fixed_test_size:
            output = interp(pred_main, (label.shape[1], label.shape[2])).asnumpy()[0]
        else:
            output = interp(pred_main, (cfg.TEST.OUTPUT_SIZE[1], cfg.TEST.OUTPUT_SIZE[0])).asnumpy()[0]

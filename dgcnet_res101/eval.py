...
def predict_sliding(net, image, tile_size, classes):
    """predict_sliding"""
    interp = mindspore.ops.ResizeBilinearV2(align_corners=True)
    ...
    for row in range(tile_rows):
        for col in range(tile_cols):
            ...
            padded_prediction = interp(padded_prediction, tile_size)[0]
...
def predict_whole(net, image, tile_size):
    ...
    prediction = interp(prediction, tile_size).cpu().data[0].numpy()
